...
ENDUSAGE
		...
		$S3syncOptions['--verbose'] = true if $S3syncOptions['--dryrun']
		...
		if(sourcePrefix !~ %r{/$})
			...
			destinationPrefix.replace(destinationPrefix + '/' + sourcePrefix.split(/(?:\/|:)/).last)
		end
		...
		localTree = Generator.new do |g|
			def S3sync.localTreeRecurse(g, prefix, path)
				...
				toAdd = Array.new
				d.each do |name|
					partialPath = path + '/' + name
					fullPath = prefix + partialPath
					if name == "." or name == ".."
						# skip
					else
				...
				dItems.each do |name|
					...
					partialPath = path + '/' + name
					fullPath = prefix + partialPath
					debug("local item #{fullPath}") unless isDirNode
					if name == "." or name == ".."
						# skip
					elsif isDirNode
			...
			# a bit of a special case for local, since "foo/" and "foo" are essentially treated the same
			localPrefixTrim = localPrefix
			if localPrefix !~ %r{/$}
				# yield the root itself first, then recurse
				g.yield(LocalNode.new(localPrefixTrim, ""))
			else
				localPrefixTrim.sub!(%r{/$}, "")
			end
				...
				while nextPage do
					...
					items.each do |item|
						if not (item.kind_of? String)
							...
							$stderr.puts "prefix found: #{partialPath}" if $S3syncOptions['--debug']
							# recurse
							s3TreeRecurse(g, bucket, prefix, partialPath) if $S3syncOptions['--recursive'] #and partialPath != '/' 
						end
					end
					items = nil
		...
		while sourceNode or destinationNode do
			$stderr.puts "source: #{sourceNode.name}" if $S3syncOptions['--debug'] and sourceNode
			$stderr.puts "dest: #{destinationNode.name}" if $S3syncOptions['--debug'] and destinationNode
	...
	class S3Node < Node
		...
		def updateFrom(fromNode)
			if fromNode.respond_to?(:stream)
				...
				begin
					s3o = S3::S3Object.new(fromNode.stream, meta)
					debug(@path)
	...
	class LocalNode < Node
		...
		def initialize(prefix, partialPath)
			@path = prefix + partialPath
			@name = (partialPath.slice(1..partialPath.length) or '')
