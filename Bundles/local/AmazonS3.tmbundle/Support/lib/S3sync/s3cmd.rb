...
#{name}  createbucket|deletebucket  <bucket>  [headers]
...
ENDUSAGE
		...
		case command
			...
			when "createbucket"
				s3cmdUsage("Need a bucket") if bucket == ''
				headers = hashPairs(ARGV[2...ARGV.length])
				$stderr.puts "create bucket #{bucket} #{headers.inspect if headers}" if $S3syncOptions['--verbose']
				S3try(:create_bucket, bucket, headers) unless $S3syncOptions['--dryrun']
			when "deletebucket"
				s3cmdUsage("Need a bucket") if bucket == ''
				headers = hashPairs(ARGV[2...ARGV.length])
